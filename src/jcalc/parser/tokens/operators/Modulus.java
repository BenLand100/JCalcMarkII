...
public class Modulus extends Operator {
    ...
    /** Creates a new instance of Modulus */
    public Modulus() {
        super("%");
    }
    ...
    public static Number modulus(Number a, Number b) {
        ...
        if (aV && bV) {
            return Value.dominant((Value)a,(Value)b).modulus((Value)a, (Value)b);
        } else if (aV) {
            return ((Value)a).modulus((Value)a,b);
        } else if (bV) {
            return ((Value)b).modulus(a,(Value)b);
        } else {
    ...
    public void genCode(ClassGen cg, ConstantPoolGen cp, InstructionList il, InstructionFactory f) {
        il.append(f.createInvoke("jcalc.parser.tokens.operators.Modulus", "modulus", new ObjectType("java.lang.Number"), new Type[] { new ObjectType("java.lang.Number"), new ObjectType("java.lang.Number") }, Constants.INVOKESTATIC));
    }
