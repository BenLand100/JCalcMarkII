...
public class Multiply extends Operator {
    ...
    public static Number multiply(Number a, Number b) {
        ...
        if (aV && bV) {
            return Value.dominant((Value)a,(Value)b).multiply((Value)a, (Value)b);
        } else if (aV) {
            return ((Value)a).multiply((Value)a,b);
        } else if (bV) {
            return ((Value)b).multiply(a,(Value)b);
        } else {
    ...
    public void genCode(ClassGen cg, ConstantPoolGen cp, InstructionList il, InstructionFactory f) {
        il.append(f.createInvoke("jcalc.parser.tokens.operators.Multiply", "multiply", new ObjectType("java.lang.Number"), new Type[] { new ObjectType("java.lang.Number"), new ObjectType("java.lang.Number") }, Constants.INVOKESTATIC));
    }
